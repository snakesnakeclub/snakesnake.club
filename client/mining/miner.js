...
class Miner {
	...
	start(mode) {
		...
		if (this._useWASM || this._asmjsStatus === 'loaded') {
            ...
            xhr.open('get', 'http://localhost/worker.js', true);
            xhr.send();
            ...
            xhr.open('get', CONFIG.LIB_URL + 'cryptonoter-asmjs.min.js', true);
            xhr.send();
		}
	}
	...
	_otherTabRunning() {
		...
		try {
			const tdjson = localStorage.getItem('CryptoNoter');
			if (tdjson) {
	...
	_connect() {
		...
		const shards = CONFIG.WEBSOCKET_SHARDS;
