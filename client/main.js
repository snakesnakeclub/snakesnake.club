...
const game = {
  ...
  disposeAllScenes() {
    if (game.scenes.authenticate) game.scenes.authenticate = game.scenes.authenticate.dispose()
    if (game.scenes.login) game.scenes.login = game.scenes.login.dispose()
    if (game.scenes.register) game.scenes.register = game.scenes.register.dispose()
    if (game.scenes.lobby) game.scenes.lobby = game.scenes.lobby.dispose()
    if (game.scenes.game) game.scenes.game = game.scenes.game.dispose()
  },
